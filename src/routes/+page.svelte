...
<ul>
    {#each data.history as { when, msg, location }}
        <li>
            {new Date(when).toISOString()} - {JSON.stringify(msg.text)} from {JSON.stringify(msg.from.first_name)} @ {JSON.stringify(location?.latitude)}, {JSON.stringify(location?.longitude)}
        </li>
    {/each}
</ul>